...
from .transformers import BertPreTrainedModel, BertModel, GPT2LMHeadModel
...
class SomDST(nn.Module):
    def __init__(self, bertpath, config, n_op, n_domain, update_id, exclude_domain=False):
        super().__init__()
        self.n_op = n_op
        self.n_domain = n_domain
        self.hidden_size = config.hidden_size
        self.encoder = Encoder.from_pretrained(bertpath, config=config, 
                                                n_op=n_op,
                                                n_domain=n_domain, 
                                                update_id=update_id, 
    ...
    def forward(self, input_ids, token_type_ids,
                ...
                max_value, op_ids=None, max_update=None, teacher=None, gen_ids=None, domain_ids=None):
        ...
        if op_ids is not None and gen_ids is not None and domain_ids is not None:
            loss_s = nn.functional.cross_entropy(state_scores.view(-1, self.n_op), op_ids.view(-1))
            loss_g = masked_cross_entropy_for_value(gen_scores.contiguous(),
                                                    gen_ids.contiguous(), 0)
            loss_d = nn.functional.cross_entropy(domain_scores.view(-1, self.n_domain), domain_ids.view(-1))

            return loss_s, loss_g, loss_d
