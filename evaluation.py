...
def model_evaluation(model, test_data, tokenizer, slot_meta, epoch, op_code='4',
    ...
    logging.info("Epoch {} op accuracy : {:.4f}".format(epoch, op_acc_score))
    logging.info("Epoch {} op F1 : {}".format(epoch, str(op_F1_score)))
    logging.info("Epoch {} op hit count : {}".format(epoch, str(op_F1_count)))
    logging.info("Epoch {} op all count : {}".format(epoch, str(all_op_F1_count)))
    ...
    logging.info("-----------------------------\n")
    path = os.path.join(args.save_dir,'pred/preds_%d.json' % epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_ckpt_path", default='checkpoint/model_best.bin', type=str)
